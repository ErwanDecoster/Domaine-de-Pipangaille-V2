...
function getTranslation(lang: Language, key: string): string {
  return (ui[lang][key as keyof typeof ui[Language]] || ui[defaultLang][key as keyof typeof ui[Language]] || key) as string;
}
  ...
  send: defineAction({
    ...
    handler: async ({ name, email, object, message, phone }, context) => {
      ...
      const validationSchema = z.object({
        name: z.string().min(1, getTranslation(lang, 'contact.validation.nameRequired')),
        email: z.string().email(getTranslation(lang, 'contact.validation.emailInvalid')),
        object: z.string().min(1, getTranslation(lang, 'contact.validation.subjectRequired')),
        message: z.string().min(1, getTranslation(lang, 'contact.validation.messageRequired')),
        phone: z.string().optional(),
      });
      
      const validationResult = validationSchema.safeParse({ name, email, object, message, phone });
      
      if (!validationResult.success) {
        const errors = validationResult.error.flatten().fieldErrors;
        throw new ActionError({
          code: 'BAD_REQUEST',
          message: getTranslation(lang, 'contact.error.title'),
          fields: errors,
        });
      }
      try {
        const { data, error } = await resend.emails.send({
          ...
          replyTo: email,
          to: import.meta.env.CONTACT_MAIL || 'contact@domaine-de-pipangaille.fr',
